...
##'
##' @param emat - exons count matrix
##' @param imat - introns count matrix
...
##' @param theta.s - option to calculate value of theta
##' @param RNA_mat - RNA count matrix for theta calculation
...
##' @param min.imat.smat.correlation - minimum required Spearman rank correlation between n and s counts of a gene
##' @param min.imat.emat.correlation - minimum required Spearman rank correlation between n and e counts of a gene
##' @param min.imat.emat.slope - minimum sloope of n~e regression
...
##' @param imat.size - pre-calculated cell sizes for the imat (unspliced) matrix
...
##' }
##' @export
gene.relative.velocity.estimates <- function(emat,imat,deltaT=1,smat=NULL,steady.state.cells=colnames(emat),kCells=10,cellKNN=NULL,kGenes=1,theta.s=FALSE, RNA_mat=NULL, old.fit=NULL,mult=1e3,min.imat.smat.correlation=0.05,min.imat.emat.correlation=0.05, min.imat.emat.slope=0.05, zero.offset=FALSE,deltaT2=1, fit.quantile=NULL, diagonal.quantiles=FALSE, show.gene=NULL, do.par=TRUE, cell.dist=NULL, emat.size=NULL, imat.size=NULL, cell.emb=NULL, cell.colors=NULL, expression.gradient=NULL,residual.gradient=NULL, n.cores=defaultNCores(), verbose=TRUE) {
  if(!all(colnames(emat)==colnames(imat))) stop("emat and imat must have the same columns (cells)");
  ...
  vg <- intersect(rownames(emat),rownames(imat));
  if(is.null(smat)) {
    emat <- emat[vg,]; imat <- imat[vg,]
  } else {
    vg <- intersect(vg,rownames(smat))
    emat <- emat[vg,]; imat <- imat[vg,]; smat <- smat[vg,]
  }
  ...
  if(is.null(imat.size)) { imat.size <- Matrix::colSums(imat); }
  emat.cs <- emat.size[colnames(emat)]/mult;
  imat.cs <- imat.size[colnames(imat)]/mult;
  ...
  if(kCells>1) {
    ...
    conv.imat <- imat %*% cellKNN[colnames(imat),colnames(imat)]
    conv.emat.cs <- (emat.cs %*% cellKNN[colnames(emat),colnames(emat)])[1,]
    conv.imat.cs <- (imat.cs %*% cellKNN[colnames(imat),colnames(imat)])[1,]
    cat("done\n")
  } else {
    conv.emat <- emat; conv.imat <- imat; cellKNN <- NULL;
    conv.emat.cs <- emat.cs; conv.imat.cs <- imat.cs;
  }
  ...
  conv.imat.norm <- t(t(conv.imat)/conv.imat.cs)

  # size-normalized counts
  emat.norm <- t(t(emat)/emat.cs)
  imat.norm <- t(t(imat)/imat.cs)
  ...
  if(kGenes>1) {
    ...
    conv.imat.norm <- scaledGeneKNN %*% conv.imat.norm;
  ...
  if(!is.null(smat)) {
    ...
    if(is.null(old.fit)) {
      ...
      sfit <- data.frame(do.call(rbind,parallel::mclapply(sn(rownames(conv.emat.norm)),function(gn) {
        df <- data.frame(i=(conv.imat.norm[gn,steady.state.cells]),e=(conv.emat.norm[gn,steady.state.cells]),s=conv.smat.norm[gn,steady.state.cells])
        sd <- lm(i~s,data=df)
        r <- with(df[df$s>0,],cor(i,s,method='spearman'),3)
  ...
  resl$conv.imat.norm <- conv.imat.norm;
  ...
  if(!is.null(show.gene)) {
    ...
    if(!is.null(cell.emb)) {
      ...
      plot(cell.emb[cc,],pch=21,col=ac(1,alpha=0.2),bg=val2col(conv.imat.norm[gn,cc],gradientPalette=expression.gradient),cex=0.8,xlab='',ylab='',main=paste(gn,'u'),axes=F); box();
    }
    do <- NULL;
    if(!is.null(smat)) { # use smat-based offsets
      df <- data.frame(i=(conv.imat.norm[gn,steady.state.cells]),e=(conv.emat.norm[gn,steady.state.cells]),o=sfit[gn,'o'])
      if(zero.offset) df$o <- 0;
    }  else { # calculate offset based on the nascent counts obsered for near-0 exonic levels
      df <- data.frame(i=(conv.imat.norm[gn,steady.state.cells]),e=(conv.emat.norm[gn,steady.state.cells]))
      o <- 0;
      df$o <- o;
      #zi <- emat[gn,steady.state.cells]==0;
      if(!zero.offset) { zi <- df$e<1/conv.emat.cs[steady.state.cells]; if(any(zi)) { o <- sum(df$i[zi])/(sum(zi)+1)} }
      df$o <- o;
      
      #table(zi)
      # if(any(zi)) {
      #   do <- lm(i~e,data=df[zi,])
      #   #summary(do)
    ...
    #browser()
    d <- lm(i~e+offset(o)+0,data=df,weights=df$e^4+df$i^4);
    ...
    plot(df$e,df$i,pch=21,bg=ac(cell.col,alpha=0.3),col=ac(1,alpha=0.1),cex=0.8,xlab='s',ylab='u',main=paste(gn,'fit'))
    if(!is.null(do)) {
    ...
    if(!is.null(fit.quantile)) {
      if(diagonal.quantiles) {
        ...
        nmax <- quantile(df$i,p=0.99)
        if(emax==0) emax <- max(max(df$e),1e-3)
        if(nmax==0) nmax <- max(max(df$i),1e-3)
        x <- df$e/emax + df$i/nmax;
      ...
      if(!is.null(smat) || zero.offset) { # use smat offset
        d <- lm(i~e+offset(o)+0,data=df,weights=pw);
      } else {
        d <- lm(i~e,data=df,weights=pw)
      }
  ...
  if(is.null(old.fit)) {
    ko <- data.frame(do.call(rbind,parallel::mclapply(sn(rownames(conv.emat.norm)),function(gn) {
      ...
      if(theta.s) {
        if(is.null(RNA_mat)) {stop("RNA count matrix should be provided for theta calculation")}
        real_e <- emat[gn,steady.state.cells]/RNA_mat[gn,steady.state.cells]
        real_e[which(real_e=="NaN")]=0
        real_i <- imat[gn,steady.state.cells]/RNA_mat[gn,steady.state.cells]
        real_i[which(real_i=="NaN")]=0
        max_e <- max(real_e)
	      if(max_e<1) {max_e=1}
        max_i <- max(real_i)
        theta_e=(max_e-1)/(2*max_e-1)
        theta_i=max_i/(2*max_i+1)
        theta=max(theta_e,theta_i)
      } else {
        theta=0.5
      }
      if(is.null(fit.quantile)) {
        #d <- lm(i~e+offset(o)+0,data=df,weights=df$e^4+df$i^4);
        d <- lm(i~e+offset(o)+0,data=df,weights=df$e^4+df$i^4);
        gama=as.numeric(coef(d)[1])*(1-theta)/theta
        alpha=mean(beta/theta*df$i)
        return(c(o=df$o[1],a=alpha,b=beta,g=gama,t=theta, r=cor(df$e,df$i,method='spearman')))
      } else {
        if(diagonal.quantiles) {
          # determine maximum ranges 
          emax <- quantile(df$e,p=0.99)
          ...
          if(!is.null(smat)) { # will use smat offset, so disregard lower quantile
            pw <- as.numeric(x>=eq[2])
            emat.lm <- df$e[which(x>=eq[2])]
            imat.lm <- df$i[which(x>=eq[2])]
          } else {
            pw <- as.numeric(x>=eq[2] | x<=eq[1])
            emat.lm <- df$e[which(x>=eq[2] | x<=eq[1])]
            imat.lm <- df$i[which(x>=eq[2] | x<=eq[1])]
          }
        } else {
          eq <- quantile(df$e,p=c(fit.quantile,1-fit.quantile))
          if(!is.null(smat) || zero.offset) { # will use smat offset, so disregard lower quantile
            pw <- as.numeric(df$e>=eq[2])
            emat.lm <- df$e[which(df$e>=eq[2])]
            imat.lm <- df$i[which(df$e>=eq[2])]
          } else {
            pw <- as.numeric(df$e>=eq[2] | df$e<=eq[1])
            emat.lm <- df$e[which(df$e>=eq[2] | df$e<=eq[1])]
            imat.lm <- df$i[which(df$e>=eq[2] | df$e<=eq[1])]
          }
        }
        if(!is.null(smat) || zero.offset) { # use smat offset
          d <- lm(i~e+offset(o)+0,data=df,weights=pw);
          gama=as.numeric(coef(d)[1])*(1-theta)/theta
          alpha=mean(beta/theta*imat.lm)
          return(c(o=df$o[1],a=alpha,b=beta,g=gama,t=theta, r=cor(df$e,df$i,method='spearman')))
        } else {
          d <- lm(i~e,data=df,weights=pw)
          # note: re-estimating offset here
          gama=as.numeric(coef(d)[2])*(1-theta)/theta
          alpha=mean(beta/theta*imat.lm)
          return(c(o=as.numeric(coef(d)[1]),a=alpha,b=beta,g=gama,t=theta, r=cor(df$e,df$i,method='spearman')))
        }
      }
  ...
  if(!is.null(smat)) {
    ...
    vi <- sfit$r > min.imat.smat.correlation
    ko <- ko[vi,]
    if(!all(vi)) cat("filtered out",sum(!vi),"out of",length(vi),"genes due to low imat-smat correlation\n")
  }
  
  full.ko <- ko;
  vi <- ko$r>min.imat.emat.correlation
  if(!all(vi)) cat("filtered out",sum(!vi),"out of",length(vi),"genes due to low imat-emat correlation\n")
  ko <- ko[vi,]
  
  vi <- ko$t/(1-ko$t)*ko$g>min.imat.emat.slope
  if(!all(vi)) cat("filtered out",sum(!vi),"out of",length(vi),"genes due to low imat-emat slope\n")
  ko <- ko[vi,]

  gamma <- ko$g; offset <- ko$o; theta <- ko$t; names(gamma) <- names(offset) <- names(theta) <- rownames(ko);
  cat("calculating RNA velocity shift ... ")
  if(kGenes>1) { # gene-convolved estimation
    # estimate M value
    ipred <- ko$o + gamma*theta/(1-theta)*conv.emat.norm[names(gamma),];
    ipred[ipred<0] <- 0;
    mval <- log2(conv.imat.norm[names(gamma),]+pcount) - log2(ipred+pcount);
    resl$mval <- mval;
    
    #resl$conv.deltaE <- t.get.projected.delta(conv.emat.norm,conv.imat.norm,gamma,offset=offset,delta=deltaT)
    ...
    am <- conv.imat.norm[rownames(mval),]-offset; am[am<0] <- 0;
    ...
    #deltaE <- t.get.projected.delta2(conv.emat.norm,conv.imat,conv.imat.cs,gamma,offset=offset,delta=deltaT)
  } else { # regular estimation
    #deltaE <- t.get.projected.delta2(conv.emat.norm,conv.imat,conv.imat.cs,gamma,offset=offset,delta=deltaT)
    deltaE <- t.get.projected.delta(conv.emat.norm,conv.imat.norm,gamma,offset=offset,theta=theta, delta=deltaT)
  }
  ...
  resl <- c(resl,list(projected=emn,current=emat.norm,cellsize=emat.cs,deltaE=deltaE,deltaT=deltaT,ko=full.ko,mult=mult,kCells=kCells));
  if(!is.null(smat)) { resl$sfit <- sfit }
  return(resl)
}

##' Estimate Region velocity using EM algorithm
##'
##' @param vel - velocity estimation(from gene.relative.velocity.estimates)
##' @param iteration - Max steps of EM iterations
##' @param deltaT - amount of time to project the cell forward
##' @param n.cores - number of cores to use
##' @param verbose - output messages about progress
##' @return a list with velocity results, including the current normalized expression state ($current), projected ($projected) over a certain time ($deltaT), unscaled transcriptional change ($deltaE), fit results ($gamma, $ko, $sfit if spanning reads were used), optional cell pooling parameters ($cellKNN, $kCells), kNN-convolved normalized matrices (conv.imat.norm and conv.emat.norm), library scale ($mult)
##' @examples
##' \dontrun{
##'  # use EM algorithm to infer parameters of analytical solution. 
##' }
##' @export
gene.EM.velocity.estimates <- function(vel,iteration=100, lower=1e-6,deltaT=1,n.cores=defaultNCores(), verbose=TRUE) {
  gn.filter <- names(vel$gamma)
  emat <- as.matrix(vel$conv.emat.norm[gn.filter,])
  imat <- as.matrix(vel$conv.imat.norm[gn.filter,])
  vd <- vel$ko$valid
  theta <- vel$ko$t[vd]
  names(theta) <- gn.filter
  para <- cbind(vel$ko$a[vd],vel$ko$b[vd],vel$ko$g[vd])
  rownames(para) <- gn.filter
  colnames(para) <- c("alpha","beta","gama")
  cat("test")
  cat("EM iteration ... ")
  para_new <- data.frame(do.call(rbind,parallel::mclapply(gn.filter,function(gn) {
    para_alone <- single.gene.EM.inference(emat[gn,],imat[gn,],para[gn,],theta[gn],lower=lower,ITR = iteration)
    return(para_alone)
  },mc.cores=n.cores,mc.preschedule=T)))
  cat("done\n")
  para_new_mat <- as.matrix(para_new)
  print(dim(para_new_mat))
  rownames(para_new_mat) <- gn.filter
  vel$time <- para_new_mat[,7:ncol(para_new_mat)]
  vel$para.new <- para_new_mat[,1:6]
  cat("calculating extrapolated cell state by EM algorithm...")
  vel.projected <- EM.get.projected.cell(vel,delta=deltaT,mult=1e3,n.cores = n.cores)
  cat("done\n")
  return(vel.projected)
}
...
##' @param imat - unspliced (nascent) count matrix
...
##' @param n.cores - number of cores to use
##' @return a list with velocity results, including the current normalized expression state ($current), projected ($projected), unscaled transcriptional change ($deltaE), fit results ($ko, $sfit), optional cell pooling parameters ($cellKNN, $kCells), kNN-convolved normalized matrices (conv.imat.norm and conv.emat.norm)
##' @examples
##' \dontrun{
...
global.velcoity.estimates <- function(emat,imat,vel,base.df,deltaT=1,smat=NULL,kGenes=15,kGenes.trim=5,smooth.kGenes=0,kCells=10,deltaT2=1,min.gene.conuts=100,min.gene.cells=20,min.intron.length=10^3.5,min.exon.length=10^2.7,top.global.pearson.deviance=3,cellKNN=NULL,cell.dist=NULL,fit.quantile=NULL, zero.offset=NULL, diagonal.quantiles=FALSE, m.pcount=5,plot.model.fit=FALSE, n.cores=defaultNCores()) {
  ...
  gi <- intersect(intersect(rownames(base.df),rownames(emat)),rownames(imat))
  emat <- emat[gi,]; imat <- imat[gi,]; 
  base.df <- base.df[gi,]
  ...
  df <- data.frame(e=rowSums(emat[rownames(base.df),]), n=rowSums(imat[rownames(base.df),]), base.df)
  ...
  vel2 <- gene.relative.velocity.estimates(emat,imat,smat=smat,kCells=kCells,kGenes=1,cell.dist=cell.dist,fit.quantile=fit.quantile,zero.offset=zero.offset,diagonal.quantiles=diagonal.quantiles)
  ko <- vel2$ko;
  cat("re-estimated offsets for",nrow(ko),"out of",nrow(emat),"genes\n")
  emat <- emat[rownames(ko),]; imat <- imat[rownames(ko),]
  ...
  if(kCells>1) {
    ...
    conv.emat <- emat; conv.imat <- imat;
  }
  
  # size estimates

  conv.emat.cs <- Matrix::colSums(conv.emat)/mult;
  conv.imat.cs <- Matrix::colSums(conv.imat)/mult;
  # size-normalized counts
  conv.emat.norm <- t(t(conv.emat)/conv.emat.cs)
  conv.imat.norm <- t(t(conv.imat)/conv.imat.cs)
  ...
  npred <- t(t(conv.emat.norm[names(sqGammaPred),]*exp(as.numeric(sqGammaPred)))*conv.imat.cs)
  # adjust for offset
  npred <- npred+ko$o
  
  mval <- log2((conv.imat[rownames(npred),]+m.pcount)/(npred+m.pcount))
  ...
  ##   df <- data.frame(n=conv.imat.norm[gn,],e=conv.emat.norm[gn,],m=2^mval[gn,],o=ko[gn,'o'])
  ...
  am <- conv.imat.norm[rownames(mmval),]-offset; am[am<0] <- 0;
...
EM.get.projected.cell <- function(vel,delta=1,mult=1e3,n.cores=defaultNCores()){
  #emm <- t(t(vel$current)*vel$cellsize)
  gn.filter <- intersect(names(vel$gamma),rownames(vel$conv.emat.norm))
  emm <- vel$conv.emat.norm[gn.filter,]
  alpha <- vel$para.new[gn.filter,1]
  beta <- vel$para.new[gn.filter,2]
  gama <- vel$para.new[gn.filter,3]
  theta <- vel$ko[gn.filter,"t"]
  names(theta) <- gn.filter
  ts <- vel$para.new[gn.filter,4]
  time <- vel$time[gn.filter,]
  #es <- (1-theta)*alpha/gama*(1-exp(-gama*ts))
  deltaE <- as.matrix(data.frame(do.call(rbind,parallel::mclapply(rownames(emm),function(gn) {
    dele1 <- (emm[gn,]-(1-theta[gn])*alpha[gn]/gama[gn])*(exp(-gama[gn]*delta)-1)
    dele2 <- emm[gn,]*(exp(-gama[gn]*delta)-1)
    dele <- c(dele1[time[gn,]<ts[gn]],dele2[time[gn,]>=ts[gn]])
    dele <- dele[names(dele1)]
    return(dele)
  },mc.cores=n.cores,mc.preschedule=T))))
  rownames(deltaE) <- rownames(emm)
  colnames(deltaE) <- colnames(emm)
  vel$deltaE <- deltaE
  deltaE <- t(t(deltaE)*vel$cellsize)
  emn <- vel$current + deltaE
  emn[emn<0] <- 0;
  cellsize <- (vel$cellsize+Matrix::colSums(emn-vel$current)/mult)
  emn.norm <- t(t(emn)/cellsize)
  vel$projected <- emn.norm
  return(vel)
}
...
# Estep: function of time inference when up-regulation
Estep.solve.time.up <- function(emat,imat,alpha,beta,gama,theta){
  mnf.st1 <- function(time,emat,imat){
    x <- emat-(1-theta)*alpha/gama*(1-exp(-time*gama))
    y <- imat-theta*alpha/beta*(1-exp(-time*beta))
    d <- x^2+y^2
    return(d)
  }
  ML.time=optim(0,mnf.st1,emat=emat,imat=imat,method="L-BFGS-B",lower=0,upper=Inf)
 # ML.time=nlminb(0,mnf.st1,emat=emat,imat=imat,lower=0,upper=Inf)
  return(ML.time$par)
}

# E step: function of time inference when down-regulation 
Estep.solve.time.down <- function(ts,emat,imat,alpha,beta,gama,theta){
  es <- (1-theta)*alpha/gama*(1-exp(-gama*ts))
  is <- theta*alpha/beta*(1-exp(-beta*ts))
  mnf.st0 <- function(time,emat,imat){
    x <- emat-es*exp(-gama*(time-ts))
    y <- imat-is*exp(-beta*(time-ts))
    d <- x^2+y^2
    return(d)
  }
  ML.time=optim(ts,mnf.st0,emat=emat,imat=imat,method="L-BFGS-B",lower=ts,upper=Inf)
 # ML.time=nlminb(ts,mnf.st0,emat=emat,imat=imat,lower=ts,upper=Inf)
  return(ML.time$par)
}

# M step: function of parameters update
Mstep.solve.para <- function(pa,emat,imat,st,t,ts,theta,lower=1e-6){
  es <- (1-theta)*pa[1]/pa[3]*(1-exp(-pa[3]*ts))
  is <- theta*pa[1]/pa[2]*(1-exp(-pa[2]*ts))
  para.max <- function(pa,emat,imat){
    sumd=0
    for(i in 1:length(emat)) {
      if(st[i]==1) {
        x <- emat[i]-(1-theta)*pa[1]/pa[3]*(1-exp(-t[i]*pa[3]))
        y <- imat[i]-theta*pa[1]/pa[2]*(1-exp(-t[i]*pa[2]))
        sumd <- sumd+x^2+y^2
      }
      else {
        x <- emat[i]-es*exp(-pa[3]*(t[i]-ts))
        y <- imat[i]-is*exp(-pa[2]*(t[i]-ts))
        sumd <- sumd+x^2+y^2
      }
    }
    return(sumd)
  }
  ML.para <- optim(pa,para.max,emat=emat,imat=imat,method = "L-BFGS-B",lower = lower,upper = Inf)
  dif <- ML.para$value
  pa[1] <- ML.para$par[1]
  pa[2] <- ML.para$par[2]
  pa[3] <- ML.para$par[3]
  ...
  n <- length(emat)
  Likehood <- 1/2/pi/dif*exp(-1/n/2/dif*sumd)
  ML.para$likehood <- Likehood
  return(ML.para)
}

# EM algorithm to infer parameters for single gene
single.gene.EM.inference <- function(emat,imat,para,theta,lower=1e-6,ITR=100){
  itr=ITR
  max.itr=itr
  alpha=para[1]
  beta=para[2]
  gama=para[3]
  t <- replicate(length(emat),0)
  st <- vector("integer",length(emat))
  names(t) <- names(emat)
  while(itr>0){
    st[beta*imat/theta<alpha] <- 1
    for (i in 1:length(emat)) {
      if(st[i]==1) {t[i]=Estep.solve.time.up(emat[i],imat[i],alpha,beta,gama,theta)}
    }
    ts <- max(t)
    for (i in 1:length(emat)) {
      if(st[i]==0) {t[i]=Estep.solve.time.down(ts,emat[i],imat[i],alpha,beta,gama,theta)}
    }
    ML.para <- Mstep.solve.para(c(alpha,beta,gama),emat,imat,st,t,ts,theta,lower=lower)
    likehood <- ML.para$likehood
    if(ML.para$par[1]==lower || ML.para$par[2]==lower || ML.para$par[3]==lower){
      break
    } else if(abs(ML.para$par[1]-alpha)/alpha<0.01 && abs(ML.para$par[2]-beta)/beta<0.01 && abs(ML.para$par[3]-gama)/gama<0.01){
      break
    } else {
      alpha <- ML.para$par[1]
      beta <- ML.para$par[2]
  ...
  npara <- c(alpha,beta,gama,ts=ts,ITR=max.itr+1-itr,Likehood=likehood)
  rv <- c(npara,t)
  return(rv)
}
